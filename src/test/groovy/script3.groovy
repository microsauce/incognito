...
def timingReport = [:] as LinkedHashMap
...
for (def i in 0..0) {
    ...
    jruby([kid: groovy_kid[JRUBY]], '''
        ...
        puts "before call groovy method with ruby callback:"
        kid.acceptCallback('Jimmy John') do |some_str|
          puts "\tmy favorite sandwhich shop: #{some_str}"
        end
        puts "after call groovy method with ruby callback."
    ...
    timingReport[i] = System.currentTimeMillis() - startTime
}

println "\n\nExecution time of each iteration:"
timingReport.each { iteration, time ->
